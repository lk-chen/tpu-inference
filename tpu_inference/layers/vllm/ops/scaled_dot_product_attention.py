...
def vllm_vit_sdpa(
    ...
    When cu_seqlens is provided, attention is computed per-image (like the upstream GPU implementation)
    instead of one big batched call with segment IDs. This avoids XLA VMEM OOM when many images are
    batched together, because the flash attention kernel only ever sees one image's patches at a time.
    """
    ...
    q_pad = (MIN_BLOCK_SIZE - (q_seq_len % MIN_BLOCK_SIZE)) % MIN_BLOCK_SIZE
    kv_pad = (MIN_BLOCK_SIZE - (kv_seq_len % MIN_BLOCK_SIZE)) % MIN_BLOCK_SIZE
